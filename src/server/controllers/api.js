...
router.get('/courses', auth.ensureAuthenticated, (req, res) => {
  let donations = allDonations.filter((donation) => {
    return donation.donatorId == req.user.id
  })
  ...
  try {
    let populated = donations.map((donation) => {
      let p = allPackages.filter((p) => {
        return p.id == donation.packageId
      })[0]

      logger.debug(' -- ', p)

      let projects = p.grants.map((grantId) => {
        let g = allGrants.filter((grant) => {
...
router.get('/packages/:id', auth.ensureAuthenticated, (req, res) => {
  ...
  if (pack) {
    pack.projects = allGrants.filter((grant) => {
      return grant.packageId == req.params.id
    })

    pack.donations = allDonations.filter((donation) => {
      return (
        donation.donatorId == req.user.id && donation.packageId == req.params.id
      )
    })
  }
